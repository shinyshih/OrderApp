...
class AppDelegate: UIResponder, UIApplicationDelegate {
            ...
            DatabaseService.shared.usersReference.child(userID).observeSingleEvent(of: .value, with: { (snapshot) in
                if let value = snapshot.value as? NSDictionary, let role = value["role"] as? String {
                    ...
                    if role == "boss" {
                        ...
                        let initialViewController = (storyboard?.instantiateViewController(withIdentifier: "TabBarForEditor"))! as UIViewController
                        self.window?.rootViewController = initialViewController
                        self.window?.makeKeyAndVisible()
                        
                    } else {
                        
                        let initialViewController = (storyboard?.instantiateViewController(withIdentifier: "TabBarForUser"))! as UIViewController
                        self.window?.rootViewController = initialViewController
                        self.window?.makeKeyAndVisible()
                    }
                }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
         ...
         */
        let container = NSPersistentContainer(name: "FireBaseAppTest")
